...
define('editor_tools/scriptable_toolbar',['require'],function(require) {
  ...
  return ScriptableToolbar = (function() {
    function ScriptableToolbar(parent, config) {
      this.config = config;
      this.menuMap = {};
      this.initVisual(parent);
      this.eventQueue = this.config.eventQueue;
    }
    ...
    ScriptableToolbar.prototype.addItem = function(section, itemName, imghRef, helpText, clear) {
      ...
      item.append($("<img>").addClass("ribbon-icon").attr("src", this.config.root_path + imghRef));
      $(item).click(function() {
        var impls, script;
...
define('editor_tools/interpretter',['require'],function(req) {
  ...
  return Interpreter = (function() {
    function Interpreter(editor, config) {
      var _this;
      this.editor = editor;
      this.config = config;
      _this = this;
      this.env = {};
      this.eventQueue = this.config.eventQueue;
      JOBAD.util.on(this.eventQueue, "interpreter/getImplementation", function(item) {
